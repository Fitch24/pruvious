...
} from "../fields/field.definition.js";
import { resolveAppPath, resolveLayerPath, resolveModulePath } from "../instances/path.js";
...
import { relativeDotPruviousImport, relativeImport, write } from "../utils/fs.js";
import { isObject } from "../utils/object.js";
import { camelCase, isString, kebabCase, pascalCase } from "../utils/string.js";
import { unifyLiteralStrings } from "../utils/typescript.js";
import { getStandardCollectionNames } from "./collection.resolver.js";
import { resolveCollectionFieldOptions } from "./field-options.resolver.js";
export function generateCollections(collections, fields, ts, tsServer, tsStandard, tsDashboard) {
  ...
  const multiCollectionNames = collectionsArray.filter(({ definition }) => definition.mode === "multi").map(({ definition }) => definition.name);
  const singleCollectionNames = collectionsArray.filter(({ definition }) => definition.mode === "single").map(({ definition }) => definition.name);
  ts.newDecl(`export type CollectionName = ${unifyLiteralStrings(...Object.keys(collections))}`);
  ts.newLine(
    `export const collectionNames = [${Object.keys(collections).map((c) => `'${c}'`).join(", ")}]`
  );
  ...
  ts.newDecl(
    `export type PublicReadCollectionName = ${unifyLiteralStrings(
      ...collectionsArray.filter(({ definition }) => definition.apiRoutes.read === "public").map(({ definition }) => definition.name)
    )}`
  );
  ...
  ts.newLine(`import { defineCollection } from '${relativeImport(dotPruviousPath, definitionPath)}'`).newLine("export { defineCollection }").newLine(
    `export type { PaginateResult } from '${relativeImport(
      dotPruviousPath,
      resolveModulePath("./runtime/collections/query-builder")
    )}'`
  );
  tsServer.newDecl(`export * from './collections'`).newLine(
  ...
  for (const { definition } of collectionsArray) {
    for (const operation of [
      ...
      if (definition.apiRoutes[operation] && (definition.mode === "multi" || operation === "read" || operation === "update")) {
  ...
  ts.newDecl(
    `export const collectionCapabilities: CollectionCapability[] = [${collectionCapabilities.map((c) => `'${c}'`).join(", ")}]`
  );
  ...
  for (const { definition } of collectionsArray) {
    const fieldNames = Object.entries(definition.fields).filter(([_, { additional }]) => !additional || !additional.protected).map(([fieldName]) => fieldName);
  ...
  const resolvedFieldDefinitions = {};
  ...
  for (const { definition } of collectionsArray) {
    ...
    for (const [fieldName, { type, options, additional }] of Object.entries(definition.fields)) {
      ...
      const generatedType = population ? isString(population.type.ts) ? population.type.ts : population.type.ts({
        definition: field.definition,
        fields: fieldsMap,
        name: fieldName,
        options: resolvedOptions
      }) : field ? isString(field.definition.type.ts) ? field.definition.type.ts : field.definition.type.ts({
        definition: field.definition,
        fields: fieldsMap,
        name: fieldName,
        options: resolvedOptions
  ...
  for (const { definition } of collectionsArray) {
    ...
    const fieldsByType = Object.fromEntries(
      Object.values(fields).map(({ definition: definition2 }) => [definition2.name, []])
    );
  ...
  for (const { definition } of collectionsArray) {
    ...
    if (getModuleOption("uploads") && definition.name === "uploads") {
      ts.newLine("/**").newLine(" * Represents a `File` to upload and associate with the current input.").newLine(" */").newLine("$file: File");
    }
    ts.newLine("}");
  }
  ts.newLine("}");
  ...
  for (const { definition } of collectionsArray) {
    const componentPath = definition.dashboard.overviewTable.additionalTableRowOptionsVueComponent ? definition.dashboard.overviewTable.additionalTableRowOptionsVueComponent.startsWith("~") ? resolveModulePath(definition.dashboard.overviewTable.additionalTableRowOptionsVueComponent.replace("~", "")) : resolveAppPath(definition.dashboard.overviewTable.additionalTableRowOptionsVueComponent) : null;
    if (componentPath) {
      tsDashboard.newLine(
  ...
  for (const { definition } of collectionsArray) {
    const componentPath = definition.dashboard.additionalRecordOptionsVueComponent ? definition.dashboard.additionalRecordOptionsVueComponent.startsWith("~") ? resolveModulePath(definition.dashboard.additionalRecordOptionsVueComponent.replace("~", "")) : resolveAppPath(definition.dashboard.additionalRecordOptionsVueComponent) : null;
    if (componentPath) {
      tsDashboard.newLine(
        `'${definition.name}': () => defineAsyncComponent(() => import('${relativeDotPruviousImport(
          componentPath
  ...
  tsDashboard.newLine(`}`);
  const customFieldLayoutComponents = [];
...
function extractCustomFieldLayoutComponents(fieldLayout) {
  const components = [];
