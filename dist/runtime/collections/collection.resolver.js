...
import { isKeyOf, isObject, walkObject } from "../utils/object.js";
import { camelCase, isString, snakeCase } from "../utils/string.js";
...
export function resolveCollections() {
  ...
  const registeredStandardCollections = getModuleOption("standardCollections");
  let errors = 0;
...
function resolveCollection(filePath, records, isStandard, ignoreDuplicate = false) {
  try {
    ...
    if (validateDefaultExport("collections", "defineCollection({ ... })", exports, filePath) && validateSafeSlug({
      subject: "collection",
      prop: "name",
      ...
      if (records[exports.default.name]) {
        ...
        queueError(
          `The collection name $c{{ ${exports.default.name} }} must not exceed 63 characters when converted to snake case ($c{{ ${snakeCase(
          ...
          if (isObject(value) && isKeyOf(value, "type") && isKeyOf(value, "options")) {
            if (!isString(key)) {
              ...
              if (snakeCase(key).length > 59) {
                ...
                return 1;
              } else if (value.additional?.index && `ix_${snakeCase(exports.default.name)}_${snakeCase(key)}}`.length > 63) {
                queueError(
                ...
                return 1;
              } else if (value.additional?.unique && `ux_${snakeCase(exports.default.name)}_${snakeCase(key)}}`.length > 63) {
                queueError(
                ...
                return 1;
              } else if (exports.default.compositeIndexes?.some((indexes) => {
                ...
                return 1;
              } else if (exports.default.uniqueCompositeIndexes?.some((indexes) => {
                ...
                return 1;
              } else if (exports.default.search && Object.keys(exports.default.search).some((structure) => {
...
export function clearCachedCollection(path) {
  delete cachedCollections[path];
}
export function getStandardCollectionNames() {
  return uniqueArray(
