...
import { removeUserTokens } from "../http/auth.js";
...
export default defineEventHandler(async (event) => {
  ...
  if (userId !== event.context.auth.user.id && !event.context.auth.user.isAdmin && !getCapabilities(event.context.auth.user)["collection-users-update"]) {
    ...
    return __(event, "pruvious-server", "You don't have the necessary permissions to $operate $record", {
      ...
      record: __(event, "pruvious-server", collections.users.label.record.plural)
    });
  }
