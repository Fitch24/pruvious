...
import { sortNaturalByProp } from "../utils/array.js";
import { __ } from "../utils/server/translate-string.js";
import { capitalize, joinRouteParts } from "../utils/string.js";
import { getCapabilities } from "../utils/users.js";
export default defineEventHandler(async (event) => {
  const runtimeConfig = useRuntimeConfig();
  const userCapabilities = getCapabilities(event.context.auth.user);
  const canAccessDashboard = event.context.auth.isLoggedIn && (event.context.auth.user.isAdmin || userCapabilities["access-dashboard"]);
  const collections = {};
  for (const [collectionName, definition] of Object.entries(_collections)) {
    if (definition.apiRoutes.readMany && canAccessDashboard && (event.context.auth.user?.isAdmin || userCapabilities[`collection-${collectionName}-read`] || collectionName === "users")) {
      collections[collectionName] = {
        ...
        dashboard: {
          ...
          visible: event.context.auth.user?.isAdmin || userCapabilities[`collection-${collectionName}-read`] ? definition.dashboard.visible : false
        },
  ...
  const menu = Object.values(collections).filter(({ name, dashboard }) => name !== "uploads" && dashboard.visible).map(({ dashboard, label, mode, name }) => ({
    collection: name,
    icon: dashboard.icon,
    label: __(event, "pruvious-dashboard", capitalize(label.collection.plural, false)),
    path: joinRouteParts(runtimeConfig.public.pruvious.dashboardPrefix, "collections", name),
    priority: name === "pages" ? 0 : name === "presets" ? 1 : name === "users" ? 4 : name === "roles" ? 5 : mode === "multi" ? 3 : 6
  }));
  if (_collections.uploads && canAccessDashboard && (event.context.auth.user?.isAdmin || userCapabilities["collection-uploads-read"])) {
    menu.push({
  ...
  for (const dashboardPage of Object.values(dashboardPages)) {
    if (canAccessDashboard && (event.context.auth.user?.isAdmin || dashboardPage.capabilities.every((capability) => userCapabilities[capability]))) {
      menu.push({
        icon: dashboardIcons[dashboardPage.icon] ?? dashboardPage.icon,
        label: __(event, "pruvious-dashboard", dashboardPage.label),
  ...
  return {
    blocks: canAccessDashboard ? Object.fromEntries(
    ...
    collections,
    isCacheActive: canAccessDashboard && (event.context.auth.user?.isAdmin || userCapabilities["clear-cache"]) ? isProduction && !!runtimeConfig.pruvious.pageCache || !!await cache() : false,
