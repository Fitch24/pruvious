...
import { objectOmit } from "../utils/object.js";
...
export default defineEventHandler(async (event) => {
  ...
  const result = await query("users", event.context.language).deselect({ password: true }).where("id", event.context.auth.user.id).populate().update(objectOmit(input.data, ["capabilities", "email", "isActive", "isAdmin", "role"]));
  if (result.success) {
