...
import { query as _query } from "../collections/query.js";
import { getQueryStringParams } from "../collections/query-string.js";
import {
...
export default defineEventHandler(async (event) => {
  ...
  if (mode === "multi" && subPaths.length >= 2) {
    if (subPaths.length === 2 && subPaths[1] === "validate" && (event.method === "POST" || event.method === "PATCH")) {
      ...
      const original = await _query(collection, event.context.language).where("id", id).first();
      if (!original) {
        setResponseStatus(event, 404);
        return __(event, "pruvious-server", "Resource not found");
      }
      const duplicate = await collections[collection].duplicate({ record: original, query: _query });
      ...
      const from = await _query(collection, event.context.language).where("id", id).first();
      if (!from) {
        setResponseStatus(event, 404);
        return __(event, "pruvious-server", "Resource not found");
      }
      ...
      const to = await _query(collection, event.context.language).where("translations", from.translations).where("language", language).first();
      ...
      if (to) {
        ...
        const query = _query(collection, event.context.language).applyQueryStringParams(params).where("id", to.id);
        if (!user.isAdmin) {
          const cache = {};
          const fieldGuardErrors = {};
          for (const guard of collections[collection].guards) {
        ...
        const result = await query.create(translation);
        if (result.success) {
          removeProtectedFields(collection, result.record);
          await applyHooksAfterCreate(collection, {
            query: _query,
    ...
    if (subPaths.length === 2 && subPaths[1] === "validate" && event.method === "PATCH") {
      ...
      const errors = await _query(collection, event.context.language).validate(
        input.data,
        "update"
      );
      if (Object.keys(errors).length) {
      ...
      const from = await _query(collection, event.context.language).language(fromLanguage).read();
      const to = await _query(collection, event.context.language).language(toLanguage).read();
      if (!from) {
        setResponseStatus(event, 404);
        return __(event, "pruvious-server", "Resource not found");
      }
      ...
      const query = _query(collection, event.context.language).applyQueryStringParams(
        params
      );
      if (!user.isAdmin) {
        const cache = {};
      ...
      await applyHooksBeforeUpdate(collection, { input: translation, currentQuery: query, query: _query, user });
      const result = await query.update(translation);
      if (result.success) {
        removeProtectedFields(collection, result.record);
        await applyHooksAfterUpdate(collection, {
  ...
  if (mode === "multi") {
    const idExists = !!(id ? await _query(collection, event.context.language).where("id", id).count() : 0);
    ...
    if (operation === "create" && collections[collection].apiRoutes.create) {
      ...
      const result = await query.create(input.data);
      if (result.success) {
        removeProtectedFields(collection, result.record);
        await applyHooksAfterCreate(collection, {
          query: _query,
    ...
    if (operation === "createMany" && collections[collection].apiRoutes.createMany) {
      ...
      const result = await query.createMany(input.data);
      if (result.success) {
    ...
    if (operation === "read" && collections[collection].apiRoutes.read) {
      ...
      const query = _query(collection, event.context.language).applyQueryStringParams(objectPick(qs.params, ["select", "populate"])).where("id", id);
      if (!user?.isAdmin) {
        const cache = {};
        for (const guard of collections[collection].guards) {
      ...
      await applyHooksBeforeRead(collection, { currentQuery: query, query: _query, user });
    ...
    if (operation === "update" && collections[collection].apiRoutes.update) {
      ...
      const query = _query(collection, event.context.language).applyQueryStringParams(objectPick(qs.params, ["select", "order", "populate"])).where("id", id);
      if (!user?.isAdmin) {
        const cache = {};
        const fieldGuardErrors = {};
        for (const guard of collections[collection].guards) {
      ...
      await applyHooksBeforeUpdate(collection, { input: input.data, currentQuery: query, query: _query, user });
      const result = await query.update(input.data);
      if (result.success && result.records.length) {
        removeProtectedFields(collection, result.records);
    ...
    if (operation === "updateMany" && collections[collection].apiRoutes.updateMany) {
      ...
      if (result.success) {
        for (const record of result.records) {
          removeProtectedFields(collection, result.records);
          await applyHooksAfterUpdate(collection, { query: _query, record, recordId: record.id, user });
          if (!idSelected) delete record.id;
          await applyHooksBeforeReturnRecord(collection, { query: _query, record, user });
        }
        return result.records;
