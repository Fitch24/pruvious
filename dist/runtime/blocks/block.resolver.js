...
import { camelCase, kebabCase, titleCase } from "../utils/string.js";
import { validatePascalCase } from "../utils/validation.js";
const cachedBlockCode = {};
const cachedBlockDefinition = {};
const generate = typeof babelGenerate === "function" ? babelGenerate : babelGenerate.default;
export async function resolveBlocks(fields, skipCache = false) {
  const records = {};
...
async function resolveBlock(filePath, relativePath, records, fields, reserved = [], ignoreDuplicate = false) {
  const res = {
  ...
  try {
    ...
    if (scriptSetupAst) {
      ...
      res.code = res.code.replace(/^.*defineBlock\(/m, "export const __defineBlock = (").replace(/^.*defineBlockFields\(/m, "export const __defineProps = (");
...
`;
      ...
      records[res.definition.name] = {
        ...
        ).add(res.code).newLine(`const __defineBlockFields = (__defineBlock as any)['fields'] || {}`).newLine(
          "const __definePropsFields = Object.fromEntries(Object.entries(__defineProps).filter(([_, prop]) => (prop as any).__fromDefineProps))"
        ).newLine("const __export = {").newLine(`name: '${res.definition.name}',`).newLine(`label: (__defineBlock as any).label || '${titleCase(res.definition.name, false)}',`).newLine(`fields: { ...__defineBlockFields, ...__definePropsFields },`).newLine(`slots: (__defineBlock as any).slots ?? {},`).newLine(`description: (__defineBlock as any).description || '',`).newLine(`icon: (__defineBlock as any).icon || 'Components',`).newLine("}").newLine("export default __export as unknown as Required<BlockDefinition>").getContent()
      };
...
export async function resolveBlockDefinition(block) {
  const resolved = {
  ...
  if (!block.definition.fields && cachedBlockCode[block.source]) {
    try {
      cachedBlockDefinition[block.source] = (await evaluateModule(
...
export function clearCachedBlock(path) {
