...
import { isPascalCase, isSafeSlug, isString } from "./string.js";
...
export function validateSlug(options) {
...
export function validateSafeSlug(options) {
  if (!validateString(options)) {
    return false;
  }
  if (!isSafeSlug(options.value)) {
...
export function validatePascalCase(options) {
  if (!validateString(options)) {
    return false;
  }
...
export function validateDefaultExport(subjects, useFn, file, path) {
  if (!file.default) {
    return queueErrorAndReturn(
...
function inSuffix(path) {
  return path ? ` in $c{{ ${path} }}` : ".";
}
export function queueErrorAndReturn(message, ...args) {
