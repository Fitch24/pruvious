...
import { sortNatural, sortNaturalByProp } from "../array.js";
export class MediaSelection {
  constructor(data = { directories: [], uploads: [] }) {
    this.data = data;
  }
  directories = ref({});
  uploads = ref({});
  ...
  refresh() {
    ...
    if (!this.origin.value || this.origin.value.type === "directory" && !this.directories.value[this.origin.value.directory] || this.origin.value.type === "upload" && !this.uploads.value[this.origin.value.uploadId]) {
      this.origin.value = directoriesCount ? { type: "directory", directory: sortNatural(Object.keys(this.directories.value))[0] } : uploadsCount ? { type: "upload", uploadId: sortNaturalByProp(Object.values(this.uploads.value), "filename")[0].id } : null;
    }
    this.type.value = {
      singular: directoriesCount && uploadsCount ? __("pruvious-dashboard", "item") : directoriesCount ? __("pruvious-dashboard", "folder") : __("pruvious-dashboard", "file"),
      plural: directoriesCount && uploadsCount ? __("pruvious-dashboard", "items") : directoriesCount ? __("pruvious-dashboard", "folders") : __("pruvious-dashboard", "files")
    };
