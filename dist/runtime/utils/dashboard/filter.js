...
export class Filter extends QueryStringParams {
  ...
  fromString(queryString, checkUpdated = true) {
  ...
  setDefaultLanguage(language, updateWhere = false) {
    this.defaultLanguage = language;
    if (updateWhere) {
      if (this.whereOption.value.$and) {
        const existing = this.whereOption.value.$and.find((rule) => rule.language?.$eq);
        if (existing) {
  ...
  where(options) {
    ...
    if (this.defaultLanguage) {
      if (Object.keys(options).length === 1 && options.$and && !options.$and.some((option) => option.language)) {
  ...
  checkActive() {
    this.isActive.value = JSON.stringify(this.whereOption.value) !== (this.defaultLanguage ? JSON.stringify({ $and: [{ language: { $eq: this.defaultLanguage } }] }) : JSON.stringify({ $and: [] }));
  }
  checkUpdated() {
