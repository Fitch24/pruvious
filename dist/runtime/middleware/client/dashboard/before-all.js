...
} from "../../../composables/dashboard/dashboard.js";
import { setLanguage } from "../../../composables/language.js";
import { useUser } from "../../../composables/user.js";
import { pruviousFetch } from "../../../utils/fetch.js";
import { slugify } from "../../../utils/slugify.js";
import { joinRouteParts } from "../../../utils/string.js";
import("./access-guard.js");
import("./collection-guard.js");
import("./fetch-directories.js");
import("./uploads-directory-guard.js");
import("./fresh-guard.js");
import("./guest-guard.js");
import("./installed-guard.js");
import("./root-guard.js");
import("./read-uploads-guard.js");
...
export default addRouteMiddleware("pruvious-dashboard-before-all", async (to) => {
  ...
  if (runtimeConfig.public.pruvious.dashboardRemoveSiteStyles && !unwantedStylesRemoved) {
    for (const styleSheet of document.styleSheets) {
      const devId = styleSheet.ownerNode?.dataset?.viteDevId;
      if (isProduction || !devId?.includes("node_modules")) {
        styleSheet.ownerNode?.remove();
      }
    }
    if (isDevelopment) {
      for (const styleElement of document.querySelectorAll("style[data-vite-dev-id]")) {
    ...
    for (const { path } of dashboard.value.menu.filter(({ collection }) => !collection)) {
      ...
      if (!router.hasRoute(name)) {
        router.addRoute({
          ...
          component: dashboardPageComponentImports[path],
  ...
  if (auth.value.isLoggedIn && to.query.to) {
    return navigateTo({ path: to.query.to.toString(), query: { ...to.query, to: void 0 } }, { replace: true });
  }
});
