...
export class SSH {
  constructor(config, ignoreErrors = false) {
    this.config = config;
  ...
  async domainPointsToServer(domain) {
    if (!this.isLocal()) {
  ...
  async addSite(site, override = false) {
    ...
    const nuxtPort = 3e3 + config.count;
    let nginxConfig = await nginxConfigTemplate(!site.noWWW);
    if (site.forceWWW && !site.noWWW) {
      nginxConfig = nginxConfig.replaceAll("https://{{ domainName }}", "https://www.{{ domainName }}").replaceAll('if ($host = "www.{{ domainName }}")', 'if ($host = "{{ domainName }}")');
    }
    if (site.trailing) {
      nginxConfig = nginxConfig.replaceAll("rewrite ^/(.*)/$ /$1 permanent;", "rewrite ^([^.]*[^/])$ $1/ permanent;");
    }
    nginxConfig = nginxConfig.replaceAll("{{ domainName }}", site.domain).replace("{{ nuxtPort }}", nuxtPort.toString()).replaceAll("$", "\\$");
...
${nginxConfig}EOF`);
    ...
    if (this.isLocal()) {
      ...
      await this.exec(`sudo mkdir -p /etc/letsencrypt/live/${site.domain}`);
      const certCommand = `sudo certbot certonly --non-interactive --nginx --agree-tos -m ${config.email} -d ${site.domain}` + (site.noWWW ? "" : ` -d www.${site.domain}`);
...
${nginxConfig.replaceAll("### ", "")}EOF`
    ...
    const ecosystem = ecosystemTemplate().replaceAll("{{ domainName }}", site.domain).replaceAll("{{ port }}", nuxtPort.toString()).replaceAll("{{ database }}", `postgresql://${dbUser}:${dbPassword}@127.0.0.1:5432/${dbName}`).replaceAll("$", "\\$");
...
EOF`);
  ...
  async backupSite(site) {
    ...
    await this.exec(
      `sudo -u postgres pg_dump ${slugify(site.domain).replace(/[\.-]/g, "_").slice(0, 63)} > ~/backups/${site.domain}/${id}/database.sql`,
      /^could not change directory to /
    );
  ...
  async restoreSite(site, backupId) {
    const list = await this.listBackups(site);
    const backup = list.find((backup2) => +backup2.value === backupId);
    if (!backup) {
  ...
  async mirrorSiteUp(site, force = false) {
    ...
    if (database.startsWith("postgresql:")) {
      ...
      const apiPrefix = (await this.exec(
        `grep -o 'NUXT_PRUVIOUS_API_PREFIX:.*' ~/sites/${site.domain}.config.js | awk -F "'" '{print $2}'`
      ) || "api").replace(/^\/|\/$/g, "");
      const port = (await this.exec(`awk '/port/ {gsub(/[^0-9]/,"",$2); print $2}' ~/sites/${site.domain}.config.js`)).trim();
      const urls = [
        `http://localhost:${port}`,
  ...
  async mirrorSiteDown(site, force = false) {
    const { driveType, uploadsPath, database } = await getProjectInfo();
    if (driveType === "s3") {
      consola.error("Mirroring S3 drives is not supported yet");
      process.exit(1);
    ...
    if (database.startsWith("postgresql:")) {
      ...
      fs.writeFileSync(
        ".autoload.sql",
        fs.readFileSync(".autoload.sql", "utf-8").replace(/^SET .+/gm, "").replace(/^SELECT pg_catalog\..+/gm, "").replace(/^INSERT INTO public\./gm, "INSERT INTO ").replace(/^--( .*|$)/gm, "").split("\n").filter(Boolean).join("\n")
      );
      const dbPath = database.replace("sqlite:", "");
  ...
  async exec(command, ignoreError = false) {
    const out = await this.ssh?.execCommand(
      command.startsWith("pm2") || command.startsWith("pnpm") ? `source ~/.nvm/nvm.sh; ${command}` : command
    );
    if (out?.stderr && (!ignoreError || ignoreError !== true && !ignoreError.test(out.stderr)) && !this.ignoreErrors) {
  ...
  isLocal() {
    return this.config.host === "localhost" || this.config.host.startsWith("127.0.0.") || this.config.host.startsWith("192.168.");
  }
  info(message, ...args) {
...
async function nginxConfigTemplate(www) {
