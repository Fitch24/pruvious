...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    if (Object.values(ops).every((op) => op === null)) {
      const answers = await consola.prompt(
...
export async function addPruviousModuleToNuxtConfig(cwd) {
  ...
  if (config) {
    ...
    await writeFile(module, path.resolve(cwd ?? process.cwd(), "nuxt.config.ts"));
  }
}
export function replaceAppVue(cwd) {
  const appVuePath = path.resolve(cwd ?? process.cwd(), "app.vue");
  if (fs.existsSync(appVuePath)) {
    fs.writeFileSync(appVuePath, ["<template>", "  <NuxtPage />", "</template>", ""].join("\n"));
  }
}
export function replacePackageJsonDevScript(cwd) {
...
export function addDefaultLayout(cwd) {
